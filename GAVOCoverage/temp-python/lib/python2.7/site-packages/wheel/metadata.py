...
def handle_requires(metadata, pkg_info, key):
    ...
    if may_requires:
        ...
        for key, value in sorted(list(may_requires.items()), key=sort_key):
        ...
        if 'extras' not in metadata:
            metadata['extras'] = []
        metadata['extras'].extend([key.extra for key in list(may_requires.keys()) if key.extra])
...
def pkginfo_to_dict(path, distribution=None):
    ...
    for key in sorted(unique(k.lower() for k in list(pkg_info.keys()))):
    ...
    try:
        ...
        for group, items in sorted(ep_map.items()):
            ...
            for item in sorted(map(str, list(items.values()))):
